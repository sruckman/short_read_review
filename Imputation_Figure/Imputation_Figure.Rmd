...
# 3. Idealized Imputation Function (Unchanged)
...
create_panel_html_string <- function(panel_matrix, original_panel = NULL, is_reference = FALSE) {
  ...
  html_rows <- sapply(seq_len(nrow(panel_matrix)), function(i) {
    ...
    # Only add spacing for diploid individuals, not for the dense reference panel
...
for (case_index in seq_along(all_cases_data)) {
  ...
  if (is.null(full_matrix) || nrow(full_matrix) < 1) {
    ...
    # **USE THE NEW ARGUMENT FOR THE REFERENCE PANEL**
...
cat("\nSuccessfully generated 'imputation_visualization_final.html' with a distinct reference panel.\n\n")
